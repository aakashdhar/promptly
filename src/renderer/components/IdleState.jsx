export default function IdleState({ mode, modeLabel, onStart, onTypePrompt, polishTone, onPolishToneChange, onExpand }) {
  ...
  const ringColor = isPolish ? 'rgba(48,209,88,' : isRefine ? 'rgba(168,85,247,' : isImage ? 'rgba(245,158,11,' : isVideo ? 'rgba(251,146,60,' : isWorkflow ? 'rgba(34,197,94,' : 'rgba(10,132,255,'
  const micStroke = isRefine ? 'rgba(200,160,255,0.8)' : isImage ? 'rgba(252,211,77,0.8)' : isVideo ? 'rgba(251,146,60,0.8)' : isWorkflow ? 'rgba(74,222,128,0.8)' : 'rgba(100,180,255,1)'
  const micStrokeFaded = isRefine ? 'rgba(200,160,255,0.8)' : isImage ? 'rgba(252,211,77,0.8)' : isVideo ? 'rgba(251,146,60,0.8)' : isWorkflow ? 'rgba(74,222,128,0.8)' : 'rgba(100,180,255,0.85)'
    ...
    <div id="panel-idle" className="relative z-[1]" style={{height:'134px'}}>
      ...
      >
        ...
        <div style={{position:'absolute', left:'78px', textAlign:'left', WebkitAppRegion:'no-drag'}}>
          ...
          >
            {isPolish ? "Speak it rough — get it polished" : isRefine ? "Describe what exists, what's wrong, and what you want" : isImage ? 'Speak your image idea' : isVideo ? 'Speak your video idea' : isWorkflow ? 'Describe your automation' : '⌥ Space to speak · ⌘T to type'}
          </div>
        ...
        {isPolish ? (
          ...
          <span
            ...
            style={{
              ...
              minWidth: '80px',
              textAlign: 'center',
              background: isRefine ? 'rgba(139,92,246,0.12)' : isImage ? 'rgba(245,158,11,0.12)' : isVideo ? 'rgba(251,146,60,0.12)' : isWorkflow ? 'rgba(34,197,94,0.12)' : 'rgba(10,132,255,0.12)',
              border: isRefine ? '0.5px solid rgba(139,92,246,0.3)' : isImage ? '0.5px solid rgba(245,158,11,0.3)' : isVideo ? '0.5px solid rgba(251,146,60,0.3)' : isWorkflow ? '0.5px solid rgba(34,197,94,0.3)' : '0.5px solid rgba(10,132,255,0.25)',
              color: isRefine ? 'rgba(200,160,255,1.0)' : isImage ? 'rgba(252,211,77,0.9)' : isVideo ? 'rgba(251,146,60,0.85)' : isWorkflow ? 'rgba(74,222,128,0.9)' : 'rgba(100,180,255,0.85)',
            }}
            onClick={handleModePillClick}
          >
            {modeLabel}
