...
| `src/renderer/hooks/useWindowLayout.js` | Window expand/collapse + history/settings navigation — extracted from App.jsx. Owns `isExpanded` state + `isExpandedRef`. Uses `animateToStateRef` + `transitionRef` (ref forwarding) to avoid circular deps. | `useWindowLayout({ animateToStateRef, stateRef, setCurrentState, prevStateRef, transitionRef, STATES, STATE_HEIGHTS })` → `{ isExpanded, isExpandedRef, handleExpand, handleCollapse, openHistory, closeHistory, openSettings, closeSettings }` |
...
| `src/renderer/components/IdleState.jsx` | IDLE panel — pulse ring, mode pill, click-to-record. Top-right expand button. Mode pill click opens `ModeDropdown` via portal; window resized to 480px on open and back to 134px on close so dropdown fits below the compact bar. Props now include `onModeSelect`, `onShowShortcuts`, `onShowHistory`. | props: `mode`, `modeLabel`, `onStart`, `onTypePrompt`, `polishTone`, `onPolishToneChange`, `onExpand`, `onModeSelect`, `onShowShortcuts`, `onShowHistory` |
