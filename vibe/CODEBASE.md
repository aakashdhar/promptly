...
| `src/renderer/components/ExpandedTransportBar.jsx` | Top bar of expanded view — traffic light drag spacer (abort left, collapse right), inline-flex transport row that shrinks to content width (Pause 36px, timer, Mic/Stop 52px, mode pill, Type 36px, 0.5px divider, text block), waveform zone sized to match transport row via ResizeObserver ref. Text block shows state-aware hint text + colour dot (Listening / Generating / Iterating / Paused / Transcription failed / Not logged in / Claude timed out / Empty response / Generation failed / Type your prompt / Speak your prompt). Collapse disabled (opacity 0.3, cursor not-allowed) when `mode === 'video'` or `mode === 'workflow'` or `mode === 'email'` — same pattern for all always-expanded modes. Abort dimmed at IDLE. | props: `currentState`, `duration`, `mode`, `modeLabel`, `onStart`, `onStop`, `onStopIterate`, `onPause`, `onCollapse`, `onOpenSettings`, `onTypePrompt`, `onAbort`, `generationErrorType` |
...
| `src/renderer/utils/promptUtils.js` | Shared prompt-rendering utilities — extracted from ExpandedView.jsx refactor. getModeTagStyle includes workflow (green) and email (teal) cases. | `parseSections(text)` → `[{label, body}]` (regex supports `/` in labels); `getModeTagStyle(mode)` → `{background, color}` |
| `vitest.config.js` | Vitest test runner config — environment: node, include: tests/**/*.test.js | — |
| `tests/utils.test.js` | Unit tests for pure utility functions — 23 tests across 4 modules | `parseSections` (5 tests), `getModeTagStyle` (9: polish, refine, image, design, workflow, video, email, standard prose, unknown), `formatTime` (4), `parsePolishOutput` (5) |
