...
| `src/renderer/hooks/useImageBuilder.js` | Image builder question-flow hook — extracted from App.jsx (BL-IMG-001). Owns imageAnswers, imageQuestionIndex, imageBuiltPrompt state; all 8 image handlers; assembleImagePrompt (generate-raw call + history save); calcImageBuilderHeight. 117 lines. | `useImageBuilder({ STATES, STATE_HEIGHTS, transitionRef, isExpandedRef, originalTranscript, resizeWindow, setThinkTranscript })` → `{ imageAnswers, imageQuestionIndex, imageBuiltPrompt, handleImageSelect, handleImageNext, handleImageBack, handleImageSkip, handleImageCopyNow, handleImageStartOver, handleImageEditAnswers }` |
...
| `src/renderer/components/ImageBuilderState.jsx` | IMAGE_BUILDER state — 3-tier guided interview (13 questions: 4 Essential + 3 Important + 6 Advanced) with chip selection. Compact bar + expanded view (4-col grid + progress bar). Exports `QUESTIONS` array. 305 lines. | props: `transcript`, `questionIndex`, `answers`, `onSelect`, `onNext`, `onBack`, `onSkip`, `onCopyNow`, `isExpanded` |
| `src/renderer/components/ImageBuilderDoneState.jsx` | IMAGE_BUILDER_DONE state — assembled image prompt box, param summary (orange chips), Edit answers / Start over / Copy prompt actions. Expanded: two-column layout with prompt + param breakdown + "Optimised for" chips. 152 lines. | props: `prompt`, `answers`, `transcript`, `onEditAnswers`, `onStartOver`, `isExpanded` |
