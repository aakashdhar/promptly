...
| `src/renderer/hooks/useImageBuilder.js` | Image builder question-flow hook — extracted from App.jsx (BL-IMG-001). Owns imageAnswers, imageQuestionIndex, imageBuiltPrompt state; all 8 image handlers; assembleImagePrompt (generate-raw call + history save); calcImageBuilderHeight. `handleImageNext(customValue?)` accepts optional custom text override for inline text input answers. System prompt includes model/useCase opening-phrase routing + Nano Banana Pro / resolution instructions. | `useImageBuilder(...)` → `{ imageAnswers, setImageAnswers, imageQuestionIndex, setImageQuestionIndex, imageBuiltPrompt, assembleImagePrompt, handleImageSelect, handleImageNext, handleImageBack, handleImageSkip, handleImageCopyNow, handleImageStartOver, handleImageEditAnswers }` |
...
| `src/renderer/components/ExpandedView.jsx` | Thin orchestrator for expanded layout mode — owns `selected` + `isViewingHistory` state; renders ExpandedTransportBar + ExpandedHistoryList + ExpandedDetailPanel. Rendered by App.jsx when `isExpanded=true`. Forwards `imageBuilderProps` bundle to ExpandedDetailPanel. 102 lines. | props: `currentState`, `mode`, `modeLabel`, `duration`, `generatedPrompt`, `thinkTranscript`, `onStart`, `onCollapse`, `onPause`, `onStop`, `onStopIterate`, `onRegenerate`, `onReset`, `onIterate`, `isIterated`, `setGeneratedPrompt`, `isPolishMode`, `polishResult`, `polishTone`, `onPolishToneChange`, `onOpenSettings`, `onReuse`, `onTypingSubmit`, `onSwitchToVoice`, `onTypePrompt` |
...
| `src/renderer/components/ExpandedDetailPanel.jsx` | Right panel of expanded view — history entry detail when `isViewingHistory`; per-state content (IDLE, RECORDING, PAUSED, ITERATING, TYPING, THINKING, PROMPT_READY, IMAGE_BUILDER, IMAGE_BUILDER_DONE) otherwise. TYPING delegated to ExpandedTypingContent; PROMPT_READY delegated to ExpandedPromptReadyContent; image states rendered via ImageBuilderState/ImageBuilderDoneState with isExpanded=true; history prompt rendered via PromptSections. 339 lines. | props: `selected`, `isViewingHistory`, `currentState`, `generatedPrompt`, `thinkTranscript`, `mode`, `onRegenerate`, `onReset`, `onIterate`, `isIterated`, `setGeneratedPrompt`, `isPolishMode`, `polishResult`, `polishTone`, `onPolishToneChange`, `onReuse`, `onEntryChange`, `onTypingSubmit`, `onSwitchToVoice` |
...
| `src/renderer/components/ThinkingState.jsx` | THINKING panel — status badge, morph wave, YOU SAID. `mode` prop: shows "Assembling prompt…" when mode === 'image', "Building your prompt" otherwise. | props: `transcript`, `mode` |
...
| `src/renderer/components/ImageBuilderState.jsx` | IMAGE_BUILDER state — 3-tier guided interview (17 questions: 6 Essential + 4 Important + 7 Advanced) with chip selection. Custom text input for Background and Avoid questions. Tier 1+2 summary boxes on tier transitions. Compact bar + expanded view (4-col grid + progress bar). Exports `QUESTIONS` array. | props: `transcript`, `questionIndex`, `answers`, `onSelect`, `onNext`, `onBack`, `onSkip`, `onCopyNow`, `isExpanded` |
