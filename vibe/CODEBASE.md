...
| `src/renderer/components/EvalPanel.jsx` | Self-contained eval widget — fires evaluatePrompt IPC on mount, owns loading/ready/failed state, renders toggle button + scorecard panel. Returns null silently on failure. Two-column scorecard (Without Promptly / With Promptly) with score bars, reasons, delta, and verdict badge. | props: `{ transcript, prompt }` |
...
| `evaluate-prompt` | renderer → main | ✅ registered — spawn claudePath with dual-scoring eval prompt; 30s timeout; fence-strips before JSON.parse; returns `{ success: true, data: { rawScore, promptlyScore, rawReasons, promptlyReasons } }` or `{ success: false }` |
