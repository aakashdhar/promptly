...
> Last updated: 2026-04-28 (FEATURE-VIDEO-BUILDER)
...
| `src/renderer/App.jsx` | State machine root — all states, IPC wiring, theme, iteration flow, history, polish mode. Recording, keyboard, image-builder, and video-builder flows delegated to hooks. Bar container: `linear-gradient(135deg, #0A0A14 → #0D0A18)`. STATES includes VIDEO_BUILDER + VIDEO_BUILDER_DONE; STATE_HEIGHTS both = 860. thinkingAccentColor state controls orange accent for video THINKING. handleGenerateResult has video branch: calls runVideoPreSelection with isVideoReiteratingRef. videoBuilderProps bundle assembled and passed to ExpandedView. `abortRef` guards stale THINKING result. `handleAbort()` routes to correct cancel handler per state. Collapsed-mode abort overlay button shown for all non-IDLE/HISTORY/SETTINGS/SHORTCUTS/ERROR states. | `STATES`, `STATE_HEIGHTS`, `transition()`, `handleGenerateResult()`, `handleRegenerate()`, `handleAbort()`, `openHistory()` / `closeHistory()`. Iteration flow extracted to `useIteration.js`. Refs: `transitionRef`, `handleGenerateResultRef`, `isIterated`, `generatedPromptRef`, `modeRef`, `abortRef`. New state: `thinkingAccentColor`. |
| `src/renderer/hooks/useMode.js` | Mode localStorage wrapper hook. MODE_LABELS now includes `video: 'Video'`. | `useMode()` → `{ mode, setMode, modeLabel }` |
...
| `src/renderer/hooks/useVideoBuilder.js` | Video builder Veo 3.1 hook. Phase 1: runPreSelection (Claude JSON pre-fill → VIDEO_BUILDER). Phase 2: assembleVideoPrompt (Claude natural-language assembly → VIDEO_BUILDER_DONE). 9 chip params via VIDEO_PARAM_CONFIG. Reiterate merge: user chips preserved, AI chips refreshed, removedByUser excluded; boolean toggles refreshed. Orange THINKING accent `rgba(251,146,60,0.8)` set via setThinkingAccentColor on both phases. | `VIDEO_PARAM_CONFIG` (named export), `useVideoBuilder({ STATES, transitionRef, originalTranscript, setThinkTranscript, setThinkingLabel, setThinkingAccentColor })` → `{ videoDefaults, videoAnswers, videoBuiltPrompt, showVideoAdvanced, videoActivePickerParam, videoDialogueText, videoSettingDetail, isSaved, isReiteratingRef, runPreSelection, assembleVideoPrompt, handleVideoChipRemove, handleVideoChipAdd, handleVideoParamChange, handleVideoOpenPicker, handleVideoClosePicker, handleVideoToggleAdvanced, handleVideoConfirm, handleVideoCopyNow, handleVideoCopyPrompt, handleVideoDialogueChange, handleVideoSettingChange, handleVideoSave, handleVideoStartOver, handleVideoEditAnswers }` |
| `src/renderer/hooks/useImageBuilder.js` | Image builder all-params review hook (redesigned from tier-interview). Owns imageDefaults, imageAnswers, showAdvanced, activePickerParam, removedByUser, imageBuiltPrompt state; isReiteratingRef. Phase 1: runPreSelection (Claude JSON pre-fill → imageDefaults); Phase 2: assembleImagePrompt (Claude natural-language assembly → IMAGE_BUILDER_DONE). Chip handlers: handleChipRemove (adds to removedByUser), handleChipAdd, handleParamChange. Reiterate merge logic: user-added chips preserved, AI chips refreshed from new defaults, removedByUser values excluded. | `useImageBuilder(...)` → `{ imageDefaults, imageAnswers, showAdvanced, activePickerParam, imageBuiltPrompt, isReiteratingRef, runPreSelection, assembleImagePrompt, handleChipRemove, handleChipAdd, handleParamChange, handleOpenPicker, handleClosePicker, handleToggleAdvanced, handleConfirm, handleCopyNow, handleImageStartOver, handleImageEditAnswers }` |
...
| `src/renderer/components/ExpandedView.jsx` | Thin orchestrator for expanded layout mode — owns `selected` (starts `null`, no auto-selection) + `isViewingHistory` state; renders ExpandedTransportBar + ExpandedHistoryList + ExpandedDetailPanel. Forwards `imageBuilderProps` + `videoBuilderProps` + `thinkingLabel` + `thinkingAccentColor` to ExpandedDetailPanel. useEffect resets isViewingHistory for VIDEO_BUILDER/VIDEO_BUILDER_DONE. | props: `currentState`, `mode`, `modeLabel`, `duration`, `generatedPrompt`, `thinkTranscript`, `onStart`, `onCollapse`, `onPause`, `onStop`, `onStopIterate`, `onRegenerate`, `onReset`, `onIterate`, `isIterated`, `setGeneratedPrompt`, `isPolishMode`, `polishResult`, `polishTone`, `onPolishToneChange`, `onOpenSettings`, `onReuse`, `onTypingSubmit`, `onSwitchToVoice`, `onTypePrompt`, `thinkingLabel`, `thinkingAccentColor`, `imageBuilderProps`, `videoBuilderProps`, `onAbort` |
| `src/renderer/components/ExpandedTransportBar.jsx` | Top bar of expanded view — traffic light drag spacer (now space-between with abort button left + collapse right), transport row (pause+timer, mic/stop circle, mode pill+settings), waveform zone (60% width). Collapse button disabled (opacity 0.4, cursor not-allowed, tooltip) when `mode === 'video'`. Abort button: dimmed (opacity 0.3) at IDLE, full opacity otherwise. Handles RECORDING (red stop), ITERATING (blue stop → onStopIterate, MorphCanvas wave, blue pulse rings), THINKING (MorphCanvas), TYPING (dimmed controls). | props: `currentState`, `duration`, `mode`, `modeLabel`, `onStart`, `onStop`, `onStopIterate`, `onPause`, `onCollapse`, `onOpenSettings`, `onTypePrompt`, `onAbort` |
...
| `src/renderer/components/ExpandedDetailPanel.jsx` | Right panel of expanded view — shows empty state (clock SVG + "Select a history to view details", opacity 0.35) when `currentState === 'IDLE'` and `!selected`; history entry detail when `isViewingHistory`; per-state content (IDLE, RECORDING, PAUSED, ITERATING, TYPING, THINKING, PROMPT_READY, IMAGE_BUILDER, IMAGE_BUILDER_DONE, VIDEO_BUILDER, VIDEO_BUILDER_DONE) otherwise. THINKING block uses `thinkingLabel` + `thinkingAccentColor` props. Video states rendered via VideoBuilderState/VideoBuilderDoneState with full videoBuilderProps. | props: `selected`, `isViewingHistory`, `currentState`, `generatedPrompt`, `thinkTranscript`, `mode`, `onRegenerate`, `onReset`, `onIterate`, `isIterated`, `setGeneratedPrompt`, `isPolishMode`, `polishResult`, `polishTone`, `onPolishToneChange`, `onReuse`, `onEntryChange`, `onTypingSubmit`, `onSwitchToVoice`, `thinkingLabel`, `thinkingAccentColor`, `imageBuilderProps`, `videoBuilderProps` |
...
| `tests/utils.test.js` | Unit tests for pure utility functions — 20 tests across 4 modules | `parseSections` (5 tests), `getModeTagStyle` (6: polish, refine, image, design, standard prose, unknown), `formatTime` (4), `parsePolishOutput` (5) |
