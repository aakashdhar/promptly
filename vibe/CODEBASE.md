...
| `src/renderer/components/ExpandedView.jsx` | Three-zone expanded layout mode (POLISH-TOGGLE / BUG-TOGGLE-002/003/004) — top transport bar (record/stop/pause/waveform), left session-history panel, right state-content panel (IDLE/RECORDING/THINKING/PROMPT_READY). Left panel: search (icon → live input → `searchHistory()`), All/Saved tabs, filter chips (All/👍/👎/Unrated), stats bar (count + rating %), entry list with title fallback + ↻ iteration badge + bookmark/rating indicators, count footer, Clear all. Right panel: full parity with HistoryPanel (bookmark, rate, tag, copy, reuse). BUG-TOGGLE-004: waveform zone uses `padding: 0 20%` to contain canvas to 60% width; pulse rings use `pulse-ring` keyframe (scale 1→1.8); thinking state skeleton grouped into labelled sections with `padding: 24px 15%`. Rendered by App.jsx when `isExpanded=true`. | props: `currentState`, `mode`, `modeLabel`, `duration`, `generatedPrompt`, `originalTranscript`, `thinkTranscript`, `onStart`, `onCollapse`, `onPause`, `onStop`, `onRegenerate`, `onReset`, `onIterate`, `isIterated`, `setGeneratedPrompt`, `isPolishMode`, `polishResult`, `polishTone`, `onPolishToneChange` |
